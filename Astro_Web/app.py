        ...
        with tab4:
            ...
            if ai_mode == "包含占星及八字":
                try:
                    ...
                    ai_text += f"交運間距：出生後 {yun_obj.getStartYear()}年 {yun_obj.getStartMonth()}個月 {yun_obj.getStartDay()}天起運\n"
                    ai_text += f"起運時間：{first_dy.getStartAge()}歲 ({first_dy.getStartYear()}年)\n"
                    
                    ai_text += "\n八個大運與流年：\n"
                    for dy in da_yuns[1:9]:
                        ai_text += f"👉 [{dy.getStartAge()}歲 - {dy.getStartYear()}年] 大運干支：{dy.getGanZhi()}\n"
